class AssignmentGradesController < ApplicationController
  ...
  def index
    @assignment_grades = AssignmentGrade.order(:name)  # Sorts courses by name
    respond_to do |format|
      format.html
      format.csv { render text: @assignment_grades.to_csv }
      format.xls # { render text: @courses.to_csv(col_sep: "\t")}
      format.json
    end    
  end
  ...
  private
    ...
    def assignment_grade_params
      params.require(:assignment_grade).permit(:name, :letter_grade, :percent_grade, :student_id, :course_id)
    end
end
